            ...
            if os.path.exists(audio_info["audio_path"]):
                ...
                if file_age > max_age:
                    ...
                    try:
                        ...
                        logger.info(f"Archivo antiguo eliminado: {audio_info['audio_path']}")
                    except Exception as e:
                        logger.error(f"Error eliminando archivo {audio_info['audio_path']}: {e}")
            else:
...
def process_audio(request):
    ...
    try:
        ...
        logger.info(f"Procesamiento completado: {response_data['sound_type']} (confianza: {response_data['confidence']:.2f})")
...
def get_audio(request, audio_id):
    ...
    try:
        ...
        if audio_info["user_id"] != request.user.id:
            logger.error(f"Usuario {request.user.id} no tiene acceso al audio {audio_id}")
            return Response(
        ...
        try:
            ...
            response['Content-Disposition'] = f'attachment; filename="audio_{audio_id}.wav"'
